% consulta: resolver_diablotin(Cambios).
% parámetro Cambios = 6
...
ficha('@',0,0). ficha('A',0,1). ficha('B',0,2). ficha('C',0,3).
ficha('D',0,4). ficha('E',0,5). ficha('F',0,6).
ficha('G',1,0). ficha('H',1,1). ficha('I',1,2). ficha('J',1,3).
ficha('K',1,4). ficha('L',1,5). ficha('M',1,6).
ficha('N',2,0). ficha('O',2,1). ficha('P',2,2). ficha('Q',2,3).
ficha('R',2,4). ficha('S',2,5). ficha('T',2,6).
ficha('U',3,0). ficha('V',3,1). ficha('W',3,2). ficha('X',3,3).
ficha('Y',3,4). ficha('Z',3,5). ficha(' ',3,6).

resolver_diablotin(NumCambios) :-
    retractall(visitado(_)),
    desordenar_tablero(NumCambios, TableroDesordenado),
    format("Tablero de partida:~n", []),
    mostrar_posiciones([TableroDesordenado]),
    format("~n", []),
    ...
    format("Solucion encontrada.~n", []),
    format("Numero de movimientos: ~w~n", [N1]),
...
resolver(LimiteEvaluacion, [ [ _, TableroSiguiente] | TablerosSiguientes], [TableroSiguiente | MovimientosSolucion]) :-
    assertz(visitado(TableroSiguiente)),
...
insertar_movimientos(_, Siguientes, TablerosSiguientes, SiguientesOrdenados) :-
...
evaluar_posicion(Filas, Evaluacion) :-
    length(Filas, NF), NF1 is NF - 1, rango(0, NF1, NumFilas),
    Filas = [Fila | _], length(Fila, NC), NC1 is NC - 1, rango(0, NC1, NumColumnas),
...
rango(Desde, Hasta, [Desde | Resto]) :-
    Siguiente is Desde + 1,
    rango(Siguiente, Hasta, Resto).
...
hacer_movimiento(1, TableroEntrada, TableroMovido) :- mover_hueco_izquierda(TableroEntrada, TableroMovido).
hacer_movimiento(2, TableroEntrada, TableroMovido) :- mover_hueco_derecha(TableroEntrada, TableroMovido).
hacer_movimiento(3, TableroEntrada, TableroMovido) :- mover_hueco_arriba(TableroEntrada, TableroMovido).
hacer_movimiento(4, TableroEntrada, TableroMovido) :- mover_hueco_abajo(TableroEntrada, TableroMovido).
